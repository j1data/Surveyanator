...
cfwing = FrictionCoefficient(Rewing,Mach);
FFwing = FormFactor(wingxovercmax,wingtoverc,wingsweepangle,Mach);
...
cffuse = FrictionCoefficient(Refuse,Mach);
...
cfvert = FrictionCoefficient(Revert,Mach);
ffvert = FormFactor(vertxovercmax,verttoverc,sweepvert,Mach);
...
choriz = 1; % chord for vert in meters
horiztoverc = 0.12;
sweephoriz = 0;
Rehoriz = Reynolds(density,velocity,mu,choriz);

cfhoriz = FrictionCoefficient(Rehoriz,Mach);
ffhoriz = FormFactor(horizxovercmax,horiztoverc,sweephoriz,Mach);

...
[density, Temp, dynamicViscosity, b, v_infinite, c_wing, wingXOverC, wingTOverC, wingSweepAngle, Qwing, fuselageLength] = inputs{2,:};
...
[alat0, anot, Cl_max, W_e, W_p, n_prop, n_motor, alpha_cl_max] = inputs{18,1:8};
[Power_Max, g, R, Tsea, density_sea, a, m, altitude_max] = inputs{22,1:8};
[n_Strut_pos, n_Strut_neg] = inputs{25,1:2};

%General Calculations ---->
Mach_val = Mach(v_infinite, Temp); %meters/second

%Main Wing Calcs -->
S = b*c_wing; %(meters^2)
Re_wing = Reynolds(density,v_infinite,dynamicViscosity,c_wing);
Cf_wing = FrictionCoefficient(Re_wing,Mach_val);
FF_wing = FormFactor(wingXOverC,wingTOverC,wingSweepAngle,Mach_val);
Swet_wing = (1.977+0.52*wingTOverC)*S; %(m^2)
Cd_o_wing = Cf_wing*FF_wing*(Swet_wing/S)*Qwing;

%Fuselage Calcs -->
f = fuselageLength/(sqrt((4/pi)*(pi*(0.3)^2))); %f formula was Corrected
FF_fuse = 0.9 + (5/(f^1.5))+(f/400);
Re_fuse = Reynolds(density,v_infinite,dynamicViscosity,fuselageLength);
Swet_fuse = 2*pi*0.3*fuselageLength + 2*pi*(0.3)^2; %(m^2) %Corrected
Cf_fuse = FrictionCoefficient(Re_fuse,Mach_val);
Cd_o_fuse = Cf_fuse*FF_fuse*1*(Swet_fuse/S); %S was 2.15 ??? Corrected

%Vert tail sizing calcs -->
[svt] = TailVertCoefficient(CVT,LVT,b,S);

%Horz tail sizing calcs -->
Cavg = S/b;
[sht] = TailHorizCoefficient(cht,lht,Cavg,S);

%Vertical Stabilizer calcs -->
ReVert = Reynolds(density,v_infinite,dynamicViscosity,chordVertStab);
cfVert = FrictionCoefficient(ReVert,Mach_val);
ffVert = FormFactor(quarterChordVertStab,vertTOverC,vertSweepAngle,Mach_val);
sWettedVert = CVT/S; %(m^2)
cd_o_Vert = cfVert*ffVert*(sWettedVert/2.15)*1.05;

%Horizontal Stabilizer calcs -->
ReHoriz = Reynolds(density,v_infinite,dynamicViscosity,chordHorzStab);

cfHoriz = FrictionCoefficient(ReHoriz,Mach_val);
ffHoriz = FormFactor(chordHorzStab,horzToverC,horzSweepAngle,Mach_val);
sWettedHoriz = cht/S; %meters^2
cd_o_Horz = cfHoriz*ffHoriz*(sWettedHoriz/2.15)*1.05;

%Landing gear struts (front 2) calcs -->
cd_o_landF = frontStruts_dOverq*(frontStruts_Sfront/S);

%Landing gear struts (back 1)  calcs -->
cd_o_landB = backStrut_dOverq*(backStrut_Sfront/S);

%Landing Gear Wheels (3 wheels) calcs -->
cd_o_wheels = wheel_dOverq*(wheel_Sfront/S);
...
CL_stall = a3D*(alpha_cl_max - anot);
...
CL_SLF = W_total / (0.5 * density * (v_infinite^2) * S);
...
%AoA @SLF
alpha3D_SLF = (CL_SLF/a3D)+alat0;

%Range calculations -->
CLoverCD_max = 1/(4*K*dragBuildUp); %what is this equation
CLoverCD_3half_max = (((3*dragBuildUp)/K)^1.5)/(4*dragBuildUp); %This also needs checking

endurance = ((E_battery * n_prop * n_motor * sqrt((density * S))) / (sqrt(2) * (W_total^(3/2)))) * (CLoverCD_3half_max);
V_endurance = sqrt(((2/density) * (W_total/S)) * sqrt(K/(3 * dragBuildUp)));
range = (((E_battery * n_motor * n_prop) / W_total) * CLoverCD_max) / 1000; %km
V_maxrange =sqrt((2/density) * (W_total/S) * sqrt(K/(3 * dragBuildUp))); %m/s
%V_stall = ((2*W_total)/(density*S) * ((K/(3*dragBuildUp))^(.5)))^(.5); %m/s
V_stall = sqrt((2*W_total)/(density * S * CL_stall)); %m/s
...
v_infin_SLF = (((2*W_total)./(density_alt.*S).*(K./(3*dragBuildUp)).^(.5)).^(0.5)); %m/s
Power_req = .5.*density_alt.*((v_infin_SLF).^(3)).*S*dragBuildUp+((2*K*((W_total)^(2)))./(density_alt.*v_infin_SLF*S)); %watts
...
VMax_ROC = (((2*W_total)./(density_alt.*S)).*((K/(3*dragBuildUp))^(.5))).^(0.5); %m/s
ROC_Max = ((n_prop.*Power_avail)./(W_total))-VMax_ROC.*((1.155)./(CLoverCD_max)); %m/s
...
vel_manuv = sqrt(((2*n_Strut_pos)/(density*Cl_max))*(W_total/S));
...
output_1 = [a3D, alpha3D_SLF, AR, S, Mach_val, K, svt, sht, Cd_o_wing, Cd_o_fuse, cd_o_Vert, cd_o_Horz, cd_o_landF, cd_o_landB, cd_o_wheels, dragBuildUp];

%Part D
fprintf('The empty weight of our aircraft is %g Newtons or %g lbs \n',W_e,(W_e*0.2248089)) %Part D
fprintf('The payload weight of our aircraft is %g Newtons or %g lbs \n',W_p,(W_p*0.2248089)) %Part D
fprintf('The battery weight of our aircraft is %g Newtons or %g lbs \n',batteryWeight,(batteryWeight*0.2248089)) %Part D
fprintf('The total weight of the aircraft is %g Newtons or %g lbs \n', W_total,(W_total*0.2248089));
...
fprintf('Alpha at steady level flight is %g degrees at a CL of %g \n\n',alpha3D_SLF,CL_SLF) %Part D
output_d = [W_e, W_p, batteryWeight, frac_W_e, frac_W_p, frac_W_f, Cl_max, CL_SLF, alpha3D_SLF, CL_SLF]; %Need to add Total weight to this

%Part E
fprintf('The endurance is %g minutes or %g hours \n',endurance,(endurance/60));
fprintf('The range is %g kilometers \n',range);
fprintf('The velocity to achieve max range is %g m/s \n',V_maxrange);
fprintf('The stall v_infinite is %g m/s \n\n',V_stall);
...
function [Re] = Reynolds(density,v_infinite,mu,length)
    %Reynolds Number (density, v_infinite, dynamic viscosity, and length in %metric)
    Re=(density*v_infinite*length)/mu;
end

function [M] = Mach(v_infinite,Temp)
    %Gives the mach number from the v_infinite (m/s) and Temp (K)
    gamma = 1.4; %unitless
    R = 287; %J/(mol*K)
    a = sqrt(gamma*R*Temp); %speed of sound in m/s
    M = v_infinite/a; %Mach number
end

function [Cf] = FrictionCoefficient(Re,Mach)
    %calculation for the friction coefficient
    %   Reynolds, v_infinite, and Temp K in SI
...
function [svt] = TailVertCoefficient(cvt,lvt,b,S)
...
function [sht] = TailHorizCoefficient(cht,lht,c,S)
...
function [PU_radius,PU_turnRate,LT_radius,LT_turnRate] = TurningRad_andRate (v_infinite,g,n)
    %Pull up 
    PU_radius = (v_infinite^2)/(g*(n-1));
    PU_turnRate = (g*(n-1)) / v_infinite;
    %Level Turn
    LT_radius = (v_infinite^2)/(g*sqrt(n^2 - 1));
    LT_turnRate = v_infinite / LT_radius;
end
